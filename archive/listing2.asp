...
%>
<h1>Value Range</h1>
<p>The Value Range is our price saving option. These items are not new, they are quality refurbished. Everything is fully serviced before it is rented out and you can be assured that all products are in good working order. Please note that this range is subject to availability. The benefit of this range is the great value on technically current and sought after items for those on a budget.</p>
...
<%
     ...
     <!--   <li> //-->
      ...
      <article class="product">     			          
        <a href="product2.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "prodid=" & rsMain.Fields.Item("prodid").Value%>&Category=<%response.write(rsMain__varCat) %>">
        <img src = "<%=(rsMain.Fields.Item("imageSmall").Value)%>" alt= "<%=(rsMain.Fields.Item("type").Value)%>" /></a>
        <div class="spec"><a href="product2.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "prodid=" & rsMain.Fields.Item("prodid").Value%>&Category=<%response.write(rsMain__varCat) %>">
		<%=(rsMain.Fields.Item("Make").Value)%><br><%=(rsMain.Fields.Item("Model").Value)%><br>
